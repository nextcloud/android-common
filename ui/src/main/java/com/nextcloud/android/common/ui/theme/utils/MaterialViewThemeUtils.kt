...
constructor(schemes: MaterialSchemes, private val colorUtil: ColorUtil) :
    ...
    private fun colorTrailingIcon(scheme: DynamicScheme, icon: Drawable?) {
        icon?.colorFilter =
    ...
    fun colorMaterialButtonContent(button: MaterialButton, colorRole: ColorRole = ColorRole.PRIMARY) {
        withScheme(button) { scheme ->
            val contentColorList =
                buildColorStateList(
     ...
     */
    fun colorProgressBar(linearProgressIndicator: LinearProgressIndicator, colorRole: ColorRole = ColorRole.PRIMARY) {
     ...
     */
    fun colorProgressBar(linearProgressIndicator: LinearProgressIndicator, @ColorInt color: Int) {
        withScheme(linearProgressIndicator) { scheme ->
     ...
     * @param color The color to be used for the active indicator part.
     */
    fun colorProgressBar(circularProgressIndicator: CircularProgressIndicator, @ColorInt color: Int) {
        withScheme(circularProgressIndicator) { scheme ->
    ...
    fun colorBottomSheetBackground(bottomSheet: View, colorRole: ColorRole = ColorRole.SURFACE_CONTAINER_LOW) {
    ...
    fun colorTabLayout(tabLayout: TabLayout, scheme: DynamicScheme) {
    ...
    private fun chipOutlineColorList(scheme: DynamicScheme) = buildColorStateList(
    ...
    private fun chipOutlineFilterColorList(scheme: DynamicScheme) = buildColorStateList(
        android.R.attr.state_checked to dynamicColor.secondaryContainer().getArgb(scheme),
        -android.R.attr.state_checked to dynamicColor.outlineVariant().getArgb(scheme)
    )

    private fun chipSuggestionInputTextColorList(scheme: DynamicScheme) = buildColorStateList(
        android.R.attr.state_enabled to dynamicColor.onSurfaceVariant().getArgb(scheme),
        -android.R.attr.state_enabled to
            colorUtil.adjustOpacity(
                dynamicColor.onSurface().getArgb(scheme),
    ...
    private fun rippleColor(scheme: DynamicScheme) = buildColorStateList(
