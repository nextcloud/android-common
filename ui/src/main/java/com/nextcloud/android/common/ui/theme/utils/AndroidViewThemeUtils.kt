...
constructor(schemes: MaterialSchemes, private val colorUtil: ColorUtil) :
    ...
    fun getPrimaryColorDrawable(context: Context): Drawable = withScheme(context) { scheme ->
    ...
    fun tintPrimaryDrawable(context: Context, drawable: Drawable?): Drawable? =
        drawable?.let { tintDrawable(context, it, ColorRole.PRIMARY) }
    ...
    fun tintTextDrawable(context: Context, drawable: Drawable?): Drawable? =
        drawable?.let { tintDrawable(context, it, ColorRole.ON_SURFACE) }
    ...
    fun tintToolbarArrowDrawable(context: Context, drawerToggle: ActionBarDrawerToggle, drawable: Drawable) {
        withScheme(context) { scheme: DynamicScheme ->
            val wrap = DrawableCompat.wrap(drawable)
            wrap.colorFilter =
                BlendModeColorFilterCompat.createBlendModeColorFilterCompat(
    ...
    @JvmOverloads
    fun colorTextViewLinks(textView: TextView, colorRole: ColorRole = ColorRole.PRIMARY) {
    ...
    fun themeViewBackground(view: View, colorRole: ColorRole = ColorRole.SURFACE) {
        withScheme(view) { scheme ->
            view.setBackgroundColor(colorRole.select(scheme))
        }
    }
    ...
    private fun highlightText(textView: TextView, originalText: String, constraint: String, @ColorInt color: Int) {
    ...
    fun primaryColor(activity: Activity): Int = withScheme(activity) { scheme ->
        dynamicColor.primary().getArgb(scheme)
    }
