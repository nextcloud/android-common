...
constructor(private val context: Context) {
    ...
    fun getNullSafeColor(color: String?, @ColorInt fallbackColor: Int): Int =
        color.parseColorOrFallback { fallbackColor }

    @ColorInt
    fun getNullSafeColorWithFallbackRes(color: String?, @ColorRes fallbackColorRes: Int): Int =
        color.parseColorOrFallback { ContextCompat.getColor(context, fallbackColorRes) }

    @ColorInt
    fun getTextColor(colorText: String?, @ColorInt backgroundColor: Int): Int =
        colorText.parseColorOrFallback { getForegroundColorForBackgroundColor(backgroundColor) }

    @ColorInt
    fun getForegroundColorForBackgroundColor(@ColorInt color: Int): Int = if (isDarkBackground(color)) {
        Color.WHITE
    } else {
    ...
    fun colorToHexString(@ColorInt color: Int): String = String.format(null, "#%06X", HEX_WHITE and color)

    @ColorInt
    private fun String?.parseColorOrFallback(fallback: () -> Int): Int = if (this?.isNotBlank() == true) {
        try {
    ...
    @ColorInt
    fun adjustOpacity(color: Int, opacity: Float): Int = Color.argb(
